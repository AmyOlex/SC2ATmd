function varargout = clustergram_revised2(data,varargin)
%CLUSTERGRAM_REVISED creates a dendrogram and heat map on the same figure.
%
...
%
%   ----------------------------------------------------------------------
%   This program has been changed by Amy Olex in the Summer of 2006 to 
%   include an additional output for the 1D_clustergram function.  Changes 
%   were made on lines 258, 280 and 741.  Now the fourth output of this 
%   function is a vector of gene labels in the same order as they are on 
%   the generated heatmap.
%
%   This revised version of Clustergram is legally considered a Derivative
%   Form of the original Clustergram function and is distributed as per
%   Section 7 of the Deployment Addendum in The MathWorks, Inc. Software
%   License Agreement.  Any one who wishes to use or further modify this
%   method must do so only as permitted by the MathWorks License Agreement
%   regarding Derivative Forms of a previously copyrighted MATLAB function.
%   ----------------------------------------------------------------------
%   
% Copyright 2003-2004 The MathWorks, Inc.
% $Revision: 1.14.6.11 $   $Date: 2004/12/24 20:44:07 $
...
if clustStruct.onedimension
    [hAxes,hImage,hDen,something,hlabels] = create1DClustergram(clustStruct,hFig);
    % Amy - changed above line from [hAxes, hImage, hDen] = ...
else
...
if nargout
    ...
    varargout{2} = hImage;
    varargout{3} = hDen;
    varargout{4} = hlabels;     % Amy - added output {4} of gene labels in the same order as heatmap
end
...
% Amy - H1 is a vector of line handles, T1 is a permutation vector of the cluster
% (leaves) assignemnts for each element, in the original element order, and perm1 is the ordered
% permutation of the generated clusters from left to right or bottom to top as they appear on the dendrogram.
% I changed the second argument from 0 to 4 to see what it does to the
% algorithm.  The result was that everything got reduced to 4, including
% the number of elements.  This was not what I wanted.
...
if nargout
    ...
    varargout{3} = H1;
    varargout{4} = perm1;
    varargout{5} = ud.rowLabels;  % Amy - added output of cluster ordered gene labels.
end
...
perm2  %Amy - test line
% Amy - I change the second argument of dendrogram from 0 to 4 to see what
% it changes
