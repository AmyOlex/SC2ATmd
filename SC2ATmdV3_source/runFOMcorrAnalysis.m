function [ opti_range ] = runFOMcorrAnalysis( clust_list, data, method_list, metric, graph_title, savepath, fileformats )
%RUNFOMCORRANALYSIS A script to run and plot the correlation-biased cFOM analysis
...
%  analysis.
%
%   runFOMcorrAnalysis is a script designed to integrate all the clustering and
...
%   method, finally the cFOM scores are plotted and the graph is saved.
%
%   USAGE:  >>runFOMcorrAnalysis([2,4,6,8,10], datamatrix, ['kmeans','hierarchical'], 'euclid', 'myfilename', 'C:/')
%
...
%  Now graph the cFOM scores, and hold the graph
h = figure(1);
plot(clust_list(:),R(1:list_length,1),'b--')
hold on;

%start the legend matrix for the plots
...
title({'Adjusted correlation-FOM', graph_title})
