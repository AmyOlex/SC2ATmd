function [ opti_range ] = runFOManalysis_textonly( clust_list, data, method_list, metric, graph_title, savepath )
%RUNFOMANALYSIS_TEXTONLY A script to run the adjusted FOM2 analysis
%  This script is not used in the GUI.  It is a TEXT version only.  
%  This code only needs the data and not the labels.
%  This script runs the designated clustering methods through the FOM
%  analysis.
%
%   runFOManalysis_textonly is a script designed to integrate all the clustering and
...
%   method and saves the results to a text file.
%
%   USAGE:  >>runFOManalysis_textonly([2,4,6,8,10], datamatrix, ['kmeans','hierarchical'], 'euclid', 'myfilename', 'C:/')
%
...
%
%       graph_title:  The name that the FOM results should be saved under.  Do not
...
%           scoring in it, that came from the techincal report which can
